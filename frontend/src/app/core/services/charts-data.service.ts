...
import { CHART_DATA, CHART_LABELS } from '../constants/chart.data';
...
export class ChartsDataService {
  public getData(period: string, category: string, chart: string): any {
    ...
    switch (category) {
      case 'all':
        mergedData = {
          earnings: (CHART_DATA as any)[period]['earnings'].data,
          revenue: (CHART_DATA as any)[period]['revenue'].data,
        };

        break;
      default:
        filteredData = (CHART_DATA as any)[period][category].data;
    ...
    switch (chart) {
      case 'ngx-echarts':
        if (mergedData !== null) {
          return [
            this.formatNgxEchartsData(mergedData['earnings'], 'earnings'),
            this.formatNgxEchartsData(mergedData['revenue'], 'revenue'),
          ];
        } else {
      ...
      case 'ng2-charts':
        if (mergedData !== null) {
          return [
            this.formatNg2ChartsData(mergedData['earnings'], 'earnings'),
            this.formatNg2ChartsData(mergedData['revenue'], 'revenue'),
          ];
        } else {
      ...
      case 'ngx-charts':
        if (mergedData !== null) {
          return [
            this.formatNgxChartsData(mergedData['earnings'], 'earnings'),
            this.formatNgxChartsData(mergedData['revenue'], 'revenue'),
          ];
        } else {
